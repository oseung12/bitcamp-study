...
public class Receiver {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 포트번호: 
    //   - 호스트(host: Application을 실행하는 컴퓨터)에서 실행 중인 서버 프로그램을 구분하는 번호이다.
    //   - 1024 ~ 49151 사이의 값 사용한다.
    //      - 1 ~ 1023 사이의 포트 번호는 특정 서버가 사용하기 위해 미리 예약된 번호다.
    //      - 가증한 이 범위의 포트 번호는 사용하지 않는 것이 좋다.
    //   - 유명 프로그램의 포트 번호도 가능한 사용하지 말라.
    //      - 예) Oracle DBMS(1521), MySQL DBMS(3306) 등
    //   - 같은 컴퓨터에서 다른 프르그램이 이미 사용중인 포트 번호는 지정할 수 없다.
    //      - 포트 번호는 중복으로 사용 될 수 없다.
