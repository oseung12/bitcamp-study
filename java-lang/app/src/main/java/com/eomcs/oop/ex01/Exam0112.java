...
public class Exam0112 {
  public static void main(String[] args) {
    ...
    //
    // [JAVA_새_데이터_타입의_이름]
    // - 대문자로 시작한다. => class 이름은 대문자로 사용한다. 예시)Score
    // - 여러 단어가 결합된 이름인 경우 각 단어의 시작도 대문자로 작성한다. 예시) VirtualMachine
    // - 보통 명사형으로 짓는다.
    // 

    // 다음은 성적 정보를 저장할 메모리 구조를 클래스로 설계한 것이다.
    // 변수가 존재하는 상태가 아닌 Method Area에 올라가는 설계만 되어있는 상태.
    class Score {
      String name;
      int kor;
      int eng;
    ...
    // Heap에 만들어지는 Score의 인스턴스(객체)가 들어간다.
    // - 문법:
    //      new 클래스명();
    ...
    //   => s가 가리키는 인스턴스의 각 필드(name,kor,eng,math..) 값을 설정한다.
    //   => s 인스턴스의 필드 값을 설정한다.
    //   => s 객체의 필드 값을 설정한다.
    //   => s 의 (name,kor,eng,math..) 값을 준다.
    //
...
// - 문법:
//   레퍼런스명.항목명 = 값;
//   예) Score s = new Score();     // Score s; 라고 작성하면 null point 에러가 나타난다.
//       s.name = "홍길동";
//
